...
def split_artists(src: str, separators: set[str], ignoreList: set[str] | None = None):
    """
    ...
    {artist.lower() for artist in ignoreList}

    while i < len(src):
        # Check if any ignored artist starts at this position (case-insensitive)
        ignored_match = next(
            (src[i : i + len(ignored)] for ignored in ignoreList if src.lower().startswith(ignored.lower(), i)), None
        )
...
class TestSplitArtists(unittest.TestCase):
    ...
    def test_ignore_list(self):
        self.assertEqual(
            split_artists("Beatles; Earth, Wind & Fire; Queen", {";", "&"}, {"Earth, Wind & Fire"}),
    ...
    def test_extra_spaces(self):
        self.assertEqual(split_artists("  Beatles  ;  Queen  ", {";"}), ["Beatles", "Queen"])
    ...
    def test_ignore_list_with_separator(self):
        self.assertEqual(split_artists("Beatles; AC/DC", {"/", ";"}, {"AC/DC"}), ["Beatles", "AC/DC"])

    def test_ignore_list_at_start(self):
        self.assertEqual(split_artists("AC/DC; Beatles", {"/", ";"}, {"AC/DC"}), ["AC/DC", "Beatles"])
